...
-- ==========================================
-- 7A. Application Tables (For the Web Dashboard Login)
-- ==========================================
CREATE TABLE Role (
...
CREATE TABLE AppUser ( 
    ...
    MechanicID VARCHAR(20) NULL,
    ReviewStaffID MEDIUMINT UNSIGNED NULL,
    CONSTRAINT FK_AppUser_Role FOREIGN KEY (RoleID) REFERENCES Role(RoleID),
    CONSTRAINT FK_AppUser_Driver FOREIGN KEY (DriverID) REFERENCES Driver(DriverID),
    CONSTRAINT FK_AppUser_Mechanic FOREIGN KEY (MechanicID) REFERENCES Mechanic(MechanicID),
    CONSTRAINT FK_AppUser_Staff FOREIGN KEY (ReviewStaffID) REFERENCES SafetyStaff(ReviewStaffID),
    CONSTRAINT CHK_AppUser_FK_Consistency CHECK (
        (DriverID IS NOT NULL AND MechanicID IS NULL AND ReviewStaffID IS NULL) OR
        (DriverID IS NULL AND MechanicID IS NOT NULL AND ReviewStaffID IS NULL) OR
        (DriverID IS NULL AND MechanicID IS NULL AND ReviewStaffID IS NOT NULL) OR
        (DriverID IS NULL AND MechanicID IS NULL AND ReviewStaffID IS NULL) -- Allows Managers & Admin
    )
);

-- Seed Roles (Updated to include Admin)
INSERT INTO Role (RoleName) VALUES 
('Driver'), 
('Mechanic'), 
('Safety Staff'), 
('Fleet Manager'),   -- Focus: Vehicles, Drivers, Safety Events
